...
std::vector<CharSegment> OCR::segment(Plate plate){
    ...
    cv::threshold(input, thresholdImage, 60, 255, CV_THRESH_BINARY_INV);
	if (DEBUG)
...
std::string OCR::ocr(Plate *input){
    ...
    // ANN Classifier
    annClassifier = new ANNClassifier();

    cv::FileStorage fs = annClassifier->load_xml("OCR.xml");
    cv::Mat trainData;
    cv::Mat labelData;
    ...
    annClassifier->train(trainData, labelData);
